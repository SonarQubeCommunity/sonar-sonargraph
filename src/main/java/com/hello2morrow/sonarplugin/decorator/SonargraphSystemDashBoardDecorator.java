...
public class SonargraphSystemDashBoardDecorator implements Decorator {
  ...
  public boolean shouldExecuteOnProject(Project project) {
    return Qualifiers.PROJECT.equals(project.getQualifier()) && Utilities.areSonargraphRulesActive(profile);
  }
  ...
  private boolean getMeasures(DecoratorContext target, DecoratorContext source) {
    ...
    if (!copyMeasureFromChildContext(source, target, SonargraphInternalMetrics.SYSTEM_WORKSPACE_WARNINGS, SonargraphSimpleMetrics.WORKSPACE_WARNINGS)) {
      return false;
    }

    if (!copyMeasureFromChildContext(source, target, SonargraphInternalMetrics.SYSTEM_IGNORED_WARNINGS, SonargraphSimpleMetrics.IGNORED_WARNINGS)) {
      return false;
    }

    /** Special handling of this metric, because it is not generated by SGQ */
    copyMeasureFromChildContext(source, target, SonargraphInternalMetrics.SYSTEM_THRESHOLD_WARNINGS, SonargraphSimpleMetrics.THRESHOLD_WARNINGS, false);

    if (!copyMeasureFromChildContext(source, target, SonargraphInternalMetrics.SYSTEM_ALL_TASKS, SonargraphSimpleMetrics.TASKS)) {
      return false;
    }
  ...
  private boolean getAllAndCycleWarnings(DecoratorContext target, DecoratorContext source) {
    if (!copyMeasureFromChildContext(source, target, SonargraphInternalMetrics.SYSTEM_ALL_WARNINGS, SonargraphSimpleMetrics.ALL_WARNINGS)) {
      return false;
    }

    if (!copyMeasureFromChildContext(source, target, SonargraphInternalMetrics.SYSTEM_CYCLE_WARNINGS, SonargraphSimpleMetrics.CYCLE_WARNINGS)) {
      return false;
    }
    return true;
  }

  private boolean copyMeasureFromChildContext(DecoratorContext source, DecoratorContext target, Metric sourceMetric, Metric targetMetric) {
    return copyMeasureFromChildContext(source, target, sourceMetric, targetMetric, true);
  }

  private boolean copyMeasureFromChildContext(DecoratorContext source, DecoratorContext target, Metric sourceMetric, Metric targetMetric, boolean flagMissingMetric) {
    Measure sourceMeasure = source.getMeasure(sourceMetric);
    if (flagMissingMetric && sourceMeasure == null) {
      LOG.warn("Metric '" + sourceMetric.getDescription() + "' could not be found in module " + source.getProject().getName() + ".");
      return false;
    }
