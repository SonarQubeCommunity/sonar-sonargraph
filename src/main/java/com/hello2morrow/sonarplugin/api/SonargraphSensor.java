...
public final class SonargraphSensor implements Sensor {
  ...
  public void analyse(final Project project, SensorContext sensorContext) {
    ...
    if (report == null) {
      ...
      String reportPath = ReportFileReader.getReportFileName(project.getFileSystem().getBuildDir().getPath(),
          configuration);
  ...
  private void analyseMetricsForStructureDashbox(XsdAttributeRoot buildUnit) {
    ...
    processor.process(report, buildUnit);
    ...
    Utilities.saveExistingMeasureToContext(sensorContext, buildUnitmetrics, SonargraphStandaloneMetricNames.RELATIVE_ACD,
        SonargraphSimpleMetrics.RELATIVE_ACD, 1);
    Utilities.saveExistingMeasureToContext(sensorContext, buildUnitmetrics,
  ...
  private void analyseWarnings(Project project) {
    ...
    if (project.getQualifier() == Qualifiers.MODULE) {
      ...
      /*
       * Save overall system warnings to internal metrics. The decorator executed on the root parent module will retrieve them and store them
       * in the visible metrics.
       */
      Utilities.saveExistingMeasureToContext(sensorContext, systemMetrics, SonargraphStandaloneMetricNames.ALL_WARNINGS,
      ...
      Utilities.saveExistingMeasureToContext(sensorContext, systemMetrics,
          ...
          NO_DECIMAL);
      Utilities
      .saveExistingMeasureToContext(sensorContext, systemMetrics, SonargraphStandaloneMetricNames.IGNORED_WARNINGS,
